...
/** ChatGPT 스타일: 픽셀 단위로 크기 고정 (CSS 의존 없음) */
...
function getSizePx(size: IconProps['size']): number {
  return SIZE_PX[size ?? 'md'] ?? 14;
}

const STROKE = 1.5;
const VIEWBOX = '0 0 24 24';

const strokeSvgProps = (width: number, height: number, className?: string) => ({
  width,
  height,
  viewBox: VIEWBOX,
  fill: 'none' as const,
...
export const UploadIcon: React.FC<IconProps> = ({ className, size = 'md' }) => {
  ...
  return (
    <svg {...strokeSvgProps(px, px, className)}>
...
/** ChatGPT Attach 스타일 paperclip (Lucide Paperclip 동일 경로) */
export const AttachIcon: React.FC<IconProps> = ({ className, size = 'md' }) => {
  const px = getSizePx(size);
  return (
    <svg {...strokeSvgProps(px, px, className)}>
